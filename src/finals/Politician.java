...
public class Politician extends Robot {
    ...
    public void turn() throws Exception {
        if(rc.getConviction() <= 10 && rc.canEmpower(1)) rc.empower(1);
    ...
    public void checkEmpower(RobotInfo[] nearby) throws GameActionException {
        ...
        if (best > 0 && rc.canEmpower(bestD)) rc.empower(bestD);
            ...
            rc.empower(maxKillD);
        if(numFriendlyP > 8 && rc.senseNearbyRobots(2).length >= 7 && rc.senseNearbyRobots(2, rc.getTeam().opponent()).length > 2) {
            rc.empower(2);
        }
